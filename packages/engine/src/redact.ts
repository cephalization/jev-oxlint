...
function isStructuralString(node: AnyNode, parent: AnyNode | undefined): boolean {
  if (!parent) return false;
  const parentType: string = parent.type;
  // Type-level literals (`type K = "a" | "b"`) and import attributes are structure, not data.
  if (parentType.startsWith("TS") || parentType === "ImportAttribute") return true;
  switch (parent.type) {
    case "Property": {
    ...
    walk(this.sourceCode.ast as AnyNode, undefined, (node, parent) => {
      if (node.type === "Literal" && node.range) {
        ...
        if (typeof node.value === "string") {
          ...
          const digits = String(node.raw ?? node.value).replace(/\D/g, "").length;
